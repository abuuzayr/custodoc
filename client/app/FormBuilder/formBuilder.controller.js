(function() {
    ...
    function formBuilderCtrl($scope, $http, $window, $rootScope, $q, $compile, pdfFactory, ngProgressFactory, formBuilderFactory, $timeout, $stateParams, $state, appConfig) {
        ...
        function getForm(formId) {
            formBuilderFactory.getForm(formId)
                .then(function(res) {
                    var node;
                    var span;
                    var option;
                    ...
                    elements = formData.elements ? formData.elements : {};
                    console.log(elements) //TOFIX
        ...
        function addNewAutofillElement() {
            $http.post("http://localhost:3000/autofill/element", {
                    fieldName: vm.newAutofillElementName,
                    type: vm.newAutofillElementType
                }, {
                    headers: {
        ...
        function downloadPDF() {
            if (vm.allowCreate) {
                ...
                for (var i = 1; i <= vm.numberOfPages; i++) {
                    p = p.then(function(pageNumber) {
                        return addPagePromise(pageNumber);
                    });
                    p = p.then(function(pageNumber) {
                        return generateImagePromise(pageNumber);
                    });
                    p = p.then(function(pageNumber) {
                        return finishAddImagePromise(pageNumber);
                    });
                }
                p.then(function(pageNumber) {
                    return afterGeneratePDF();
                });
            }
        }
        ...
        function previewStart() {
            if (vm.allowCreate) {
                ...
                for (var i = 1; i <= vm.numberOfPages; i++) {
                    ...
                    p = p.then(function(pageNumber) {
                        return addToPreviewPromise(pageNumber);
                    });
                }
                p.then(function(pageNumber) {
                    return afterGeneratePDF();
