angular
			...
			for(var k=1; k<=vm.formData.length; k++){ //k is the form number
				...
				for (key in elements){
					...
					if(element.name.startsWith('background_')){
						...
						var testImageString = 'data:image/png;base64,' + '{{' + testScope + '}}';
						node.setAttribute('ng-src', testImageString); 
						node.setAttribute('ng-click', 'vm.openImageModal('+'vm.signatureName'+')');
						node.style.backgroundColor = element.backgroundColor;
						node.style.zIndex="1";
					}
	...
	vm.openModal = function(name) {
		vm.entryData[name] = '';
		vm.signatureName = name;
        vm.dialog = document.getElementById('signature-modal');
		vm.canvas = document.getElementById('signature-canvas');
