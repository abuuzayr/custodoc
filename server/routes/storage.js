...
function getFormStorage(req,res,next){
	connection.Do(function(db){
		db.collection('forms').find({companyId: req.decoded.companyId}).toArray().then(function(docs){
			req.formStorage = 0;
...
function getEntryStorage(req,res,next){
	connection.Do(function(db){
		db.collection('entries').find({companyId: req.decoded.companyId}).toArray().then(function(docs){
			req.entryStorage = 0;
			if(!docs || docs.length === 0)
				next();
			else{
...
function getAutofillStorage(req,res,next){
	connection.Do(function(db){
		db.collection('autofill').find({companyId: req.decoded.companyId}).toArray().then(function(docs){
